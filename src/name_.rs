...
impl Item1_ for Item_ {
	...
	fn can__(&self, _name:&str, _q:qv_::T_, _w:&mut World_) -> bool {true}
}
...
pub trait Item1_ : code_::Item_ {
	...
	fn hello__(&self, gd:code_::Opt_, q:qv_::T_, w:&mut World_, _ret:&mut result_::List_) -> Result2_ {
		...
		t_::o__(self.codes__()).hello__(gd, q.clone(), w, &mut ret2)?;
		let v = ret2.to_vec__();
		let name = if v.is_empty() {String::new()} else {v[0].to_string()};
		if self.can__(&name, q.clone(), w) {
			/*if let Some(q) = qv_::for__(q, |q| {
				if q.borrow_mut().tmp_ {None} else {Some(q)}
			})*/ {
				q.borrow_mut().name_ = name;
				return ok__()
			}
		}
		result2_::err2__("无法命名")
	}
}
