...
impl code_::Item_ for Item_ {
	...
	fn hello__(&self, env:&code_::Env_, wm:&mut WorldMut_, ret:&mut result_::List_) -> Result2_ {
		eval_::Item1_::hello__(self, env, wm, ret)
	}
}
...
impl eval_::Item1_ for Item_ {
	...
	fn src__(&self, s:String, src2:&mut String, once:&mut bool, q:qv_::T_, w:world_::T_, wm:&mut WorldMut_) -> Result2_ {
		super::u_::eval_::ok_src__(&s, q.clone(), w.clone(), wm);
		*once = as_ref__!(w).mods_.iter().any(|q2| as_ref__!(q2).src_ == as_ref__!(q).src_);
		if *once {
			return ok__()
		}
		super::u_::eval_::src__(src2, q, w, wm)
	}
}
